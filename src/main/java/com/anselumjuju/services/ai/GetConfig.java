package com.anselumjuju.services.ai;

import java.util.*;

import com.anselumjuju.lib.EnvConfig;
import com.google.genai.Client;
import com.google.genai.types.GenerateContentResponse;
import com.google.gson.Gson;
import com.google.gson.reflect.TypeToken;

public class GetConfig {
    public static List<Map<String, Object>> getConfig(Map<String, Object> uploadResponse) {
        String tableName = uploadResponse.get("tableName").toString();
        Map<String, Object> tableSchema = (Map<String, Object>) uploadResponse.get("tableSchema");

        String prompt = GetPrompt.getPrompt(tableName, tableSchema);

        try (Client client = Client.builder().apiKey(EnvConfig.GEMINI_API_KEY).build()) {
//            GenerateContentResponse response = client.models.generateContent("gemini-3-flash-preview", prompt, null);

//            String text = response.text() + " ";
//            String jsonContent = text.replaceAll("```json", "").replaceAll("```", "");

            String jsonContent = """
                    [
                      {
                        "baseTableName": "table_5adb271cd9",
                        "title": "Executive Financial Overview",
                        "description": "A high-level summary of total revenue, profitability, and order volume across the organization.",
                        "reportType": "summary",
                        "axisColumns": [
                          {
                            "type": "summarize",
                            "columnName": "Sales",
                            "operation": "sum"
                          },
                          {
                            "type": "summarize",
                            "columnName": "Profit",
                            "operation": "sum"
                          },
                          {
                            "type": "summarize",
                            "columnName": "Order ID",
                            "operation": "distinctCount"
                          },
                          {
                            "type": "summarize",
                            "columnName": "Quantity",
                            "operation": "sum"
                          }
                        ],
                        "userFilters": [
                          {
                            "tableName": "table_5adb271cd9",
                            "columnName": "Order Date",
                            "operation": "relative"
                          },
                          {
                            "tableName": "table_5adb271cd9",
                            "columnName": "Region",
                            "operation": "actual"
                          }
                        ]
                      },
                      {
                        "baseTableName": "table_5adb271cd9",
                        "title": "Monthly Sales and Profit Growth Trends",
                        "description": "Visualizes the correlation between revenue and profit growth over time to identify seasonal patterns.",
                        "reportType": "chart",
                        "chartType": "line with points",
                        "axisColumns": [
                          {
                            "type": "xAxis",
                            "columnName": "Order Date",
                            "operation": "monthYear"
                          },
                          {
                            "type": "yAxis",
                            "columnName": "Sales",
                            "operation": "sum"
                          },
                          {
                            "type": "yAxis",
                            "columnName": "Profit",
                            "operation": "sum"
                          }
                        ],
                        "isAxisMerge": "false",
                        "userFilters": [
                          {
                            "tableName": "table_5adb271cd9",
                            "columnName": "Category",
                            "operation": "actual"
                          }
                        ]
                      }
                    ]
                    """;

            Gson gson = new Gson();
            List<Map<String, Object>> configs = gson.fromJson(
                    jsonContent,
                    new TypeToken<List<Map<String, Object>>>() {
                    }.getType()
            );

//            Validate the configs before returning
            List<Map<String, Object>> validatedConfigs = ValidateConfigs.validateConfigs(tableName, tableSchema, configs);
            return validatedConfigs;
        } catch (Exception e) {
            return null;
        }
    }
}
